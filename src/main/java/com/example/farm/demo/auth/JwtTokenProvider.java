...
public class JwtTokenProvider {
    ...
    public TokenInfo generateToken(Authentication authentication){
        ...
        String refreshToken = Jwts.builder().setExpiration(refreshTokenExpiresIn).signWith(key,SignatureAlgorithm.HS256).compact();
        
        return TokenInfo.builder().grantType("Bearer").accessToken(accessToken).refreshToken(refreshToken).build();
    
    }

    // JWT 토큰을 복호화하여 토큰에 담긴 정보를 꺼냄
    public Authentication getAuthentication(String accessToken){
        ...
        //UserDetails 객체를 만들어서 Authentication 객체를 리턴
